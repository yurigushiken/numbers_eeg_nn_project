...
class SpectrogramDataset(Dataset):
    ...
    def __init__(self, root: str | Path, label_col: str = 'landing_digit',
                 transform=None, *, in_memory: bool = False):
        root = Path(root)
        ...
        # ------------------------------------------------------------------
        # Optional: preload all spectrogram tensors into RAM to eliminate
        # per-sample disk I/O.  Only enabled when `in_memory=True`.
        # ------------------------------------------------------------------

        self._cache = None
        if in_memory:
            print("[INFO] Pre-loading spectrogram tensors into RAM …", flush=True)
            tic = time.perf_counter()
            self._cache = [
                np.load(self.root / fp, mmap_mode=None)
                for fp in self.meta['filepath']
            ]
            total_mb = sum(arr.nbytes for arr in self._cache) / 2**20
            print(f"[INFO]   loaded {len(self._cache)} tensors  (~{total_mb:,.1f} MiB) in {time.perf_counter()-tic:.1f}s",
                  flush=True)
        ...
        # Add `num_channels` attribute for `timm` compatibility
        self.num_channels = self._get_channel_count()

    def _get_channel_count(self):
        """Inspects the first spectrogram to determine channel count."""
        if self._cache:
            return self._cache[0].shape[0]
        else:
            first_fp = self.root / self.meta.iloc[0]['filepath']
            return np.load(first_fp).shape[0]
    ...
    def __getitem__(self, idx):
        ...
        # Use RAM cache when available, else fall back to disk load
        if self._cache is not None:
            arr = self._cache[idx]
        else:
            arr = np.load(self.root / row['filepath'])  # (128,H,W)
