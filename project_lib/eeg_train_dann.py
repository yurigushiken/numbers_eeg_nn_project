...
try:
    from project_lib.models.cwa_transformer import CwaTransformer  # local minimal impl
except Exception:
    CwaTransformer = None

try:
    from project_lib.models.multi_scale_cnn import MultiScaleCNN
except Exception:
    MultiScaleCNN = None

try:
    from project_lib.models.eegnet_se import EEGNetWithSE
except Exception:
    EEGNetWithSE = None

try:
    from project_lib.models.eegnet_se_dann import EEGNetSE_DANN
except Exception:
    EEGNetSE_DANN = None
...
CANONICAL_DEFAULTS = {
    ...
    "dann_lambda": 1.0  # Weight for the domain loss in DANN
}
...
def run_loso(cfg: Dict[str, Any], label_fn: Callable[[Any], Any], trial: Optional['optuna.Trial']=None) -> float:
    ...
    for fold, (tr_idx, va_idx) in enumerate(fold_iter):
        ...
        elif model_name == "multi_scale_cnn":
            if MultiScaleCNN is None:
                raise ImportError("MultiScaleCNN model not found. Check code/models/multi_scale_cnn.py")
            print(f"Using MultiScaleCNN with kernels: {cfg.get('kernel_sizes')}")
            model = MultiScaleCNN(
                chunk_size=T,
                num_electrodes=C,
                num_classes=num_cls,
                ...
                F2=cfg.get('F2', 16),
                D=cfg.get('D', 2),
                kernel_2=cfg.get('kernel_2', 16),
                dropout=cfg.get('dropout', 0.5)
            ).to(DEVICE)
        elif model_name == "eegnet_se":
            if EEGNetWithSE is None:
                raise ImportError("EEGNetWithSE model not found. Check code/models/eegnet_se.py")
            print(f"Using EEGNetWithSE with SE reduction ratio: {cfg.get('se_reduction_ratio')}")
            model = EEGNetWithSE(
                num_classes=num_cls,
                num_electrodes=C,
                chunk_size=T,
                ...
                D=cfg.get('D', 2),
                F2=cfg.get('F2', 16),
                kernel_1=cfg.get('kernel_1', 64),
                kernel_2=cfg.get('kernel_2', 16),
                dropout=cfg.get('dropout', 0.5)
            ).to(DEVICE)
        elif model_name == "eegnet_se_dann":
            if EEGNetSE_DANN is None:
                raise ImportError("EEGNetSE_DANN model not found. Check code/models/eegnet_se_dann.py")
            print("Using EEGNetSE_DANN model.")
            # The DANN model needs to know how many subjects are in the training set for this fold
            num_subjects_train = len(np.unique(groups[tr_idx]))
            model = EEGNetSE_DANN(
                num_classes=num_cls,
        ...
        if run_dir is not None:
            ...
            plot_confusion(
                ...
                title=f"{cfg.get('task','').replace('_',' ')} · Fold {fold+1}  ·  Acc {best_acc:.1f}%  (ep {best_epoch}/{len(tr_history)})\nLeft-out subject {va_subject}",
