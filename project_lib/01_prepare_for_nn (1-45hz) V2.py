...
HAPPE_SET_DIR = os.path.join(BASE_INPUT_DIR, "5 - processed (1-45hz)")
HAPPE_USABLE_TRIALS_FILE = os.path.join(BASE_INPUT_DIR, "6 - quality_assessment_outputs (1-45hz)", "HAPPE_dataQC_30-08-2025 - 1-45hz.csv")

# --- DYNAMIC OUTPUT PATH ---
if PROCESS_ACC_ONLY:
    output_dir = os.path.join(BASE_OUTPUT_DIR, "acc_1_dataset (1-45hz) V2") 
else:
    output_dir = os.path.join(BASE_OUTPUT_DIR, "all_trials_dataset (1-45hz) V2")
...
for subject_id in subject_ids_in_folder:
    try:
        ...
        kept_val = str(subject_row['Kept_Segs_Indxs']).strip()
        if kept_val.lower() == "all":
            kept_indices_1based = list(range(1, len(epochs) + 1))
        else:
            kept_indices_1based = [
                int(tok) for tok in re.split(r"[\s,]+", kept_val) if tok.strip().isdigit()
            ]
