"""Landing-digit decoding for large numbers (4, 5, 6) from large primes (change trials only).
...
both the prime and the final stimulus were from the large number group {4, 5, 6}.
"""
...
def label_fn(meta: pd.DataFrame):
    """
    Return landing digit (4, 5, or 6) for non-cardinality trials where both
    prime and stimulus are in {4, 5, 6}.
    """
    ...
    # excluding cardinality trials (e.g., 44, 55, 66).
    CONDITIONS = [
        # Landing on 4
        54, 64,
        # Landing on 5
