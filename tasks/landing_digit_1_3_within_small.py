"""Landing-digit decoding for small numbers (1, 2, 3) from small primes (change trials only).

Predict the ones-place digit (1, 2, or 3) of the experimental **Condition**
code. This task filters the dataset to only include non-cardinality trials where
both the prime and the final stimulus were from the small number group {1, 2, 3}.
"""
...
def label_fn(meta: pd.DataFrame):
    """
    Return landing digit (1, 2, or 3) for non-cardinality trials where both
    prime and stimulus are in {1, 2, 3}.
    """
    ...
    # excluding cardinality trials (e.g., 11, 22, 33).
    CONDITIONS = [
        # Landing on 1
        21, 31,
        # Landing on 2
