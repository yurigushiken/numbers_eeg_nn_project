...
"""02_train_decoder_dec1_binary.py
Binary EEGNet decoder: detects whether the transition is a **decrement of 1**
(e.g., 21, 32, 43, 54, 65) versus any other transition.
Uses accuracy-only dataset, Z-score features, data augmentation, LOSO CV.
Generates confusion matrices with diagonal (black) and max-row (red) boxes and
saves all artefacts with script name in filenames.
"""
...
def parse_args():
    pr=argparse.ArgumentParser(description="dec1 binary decoder YAML")
    pr.add_argument('--cfg')
...
DATASET_DIR=Path(cfg['dataset_dir']); BATCH_SIZE=cfg['batch_size']; LR=cfg['lr']; EPOCHS=cfg['epochs']; EARLY_STOP=cfg['early_stop']; NOISE_STD=cfg['noise_std']; CHANNEL_DROPOUT_P=cfg['channel_dropout_p']; MAX_FOLDS=cfg['max_folds']
...
def load_epochs():
    ...
    for fp in files:
        ...
        if ep.metadata is None: ep.metadata=mne.create_info([],0)
        ep.metadata['subject']=sid; eps.append(ep)
    all_ep=mne.concatenate_epochs(eps)
    cond=all_ep.metadata['Condition'].astype(str)
    all_ep.metadata['dec1']=(cond.isin(DEC_SET)).astype(int)
    return all_ep
...
def build_tensors(ep):
    ...
    y=torch.from_numpy(ep.metadata['dec1'].astype(int).values).long()
...
if __name__=='__main__':
    ...
    ds=TensorDataset(X_t,y_t); num_classes=2
    log_f=open(RUN_DIR/f"logs_{SCRIPT_NAME}.csv","w",newline=""); cw=csv.writer(log_f); cw.writerow(["fold","epoch","tr","val","acc"])
    cm_total=np.zeros((2,2),int); accs=[]
    for fold,(tr_idx,val_idx) in enumerate(LeaveOneGroupOut().split(np.zeros(len(ds)), np.zeros(len(ds)), groups)):
        train_ld=DataLoader(Subset(ds,tr_idx),batch_size=BATCH_SIZE,shuffle=True)
        val_ld=DataLoader(Subset(ds,val_idx),batch_size=BATCH_SIZE,shuffle=False)
        ...
        for ep in range(1,EPOCHS+1):
            ...
            if val_l<best_v: best_v,best_a,val_l
            else: pat+=1
        ...
        ax = sns.heatmap(cm_pct, annot=annot_pct, fmt='.1f', cmap='Blues', xticklabels=['other','dec1'], yticklabels=['other','dec1'])
    ...
    plt.figure(figsize=(4,4))
    ax = sns.heatmap(cm_total_pct, annot=annot_tot, fmt='.1f', cmap='Blues', xticklabels=['other','dec1'], yticklabels=['other','dec1'])
    plt.xticks(rotation=0); plt.yticks(rotation=0)
    for i in range(2):
        ax.add_patch(Rectangle((i,i),1,1,fill=False,edgecolor='black',lw=1.5))
    ...
    recalls=np.divide(cm_total.diagonal(), cm_total.sum(axis=1), out=np.zeros(2), where=cm_total.sum(axis=1)!=0)
    rpt=[
        "Decrement-by-1 EEG decoding report",
        f"Run ID        : {RUN_ID}",
        "",
        "--- Hyper-parameters ---",
        ...
        f"  Channel dropout P   : {CHANNEL_DROPOUT_P}",
        "",
        "--- Results ---",
        f"Mean LOSO accuracy : {mean_acc:.2f}% (±{std_acc:.2f}%)",
        "Chance level      : 50.00% (binary)",
        "",
        "Recall (%):",
        f"  other : {recalls[0]*100:.1f}% (n={cm_total.sum(axis=1)[0]})",
        f"  dec1  : {recalls[1]*100:.1f}% (n={cm_total.sum(axis=1)[1]})",
        "",
        "Confusion matrix totals (rows = true):",
        f"  [[{cm_total[0,0]}, {cm_total[0,1]}],",
        f"   [{cm_total[1,0]}, {cm_total[1,1]}]]"
    ]
    with open(RUN_DIR / f'report_{SCRIPT_NAME}.txt','w') as fp: fp.write("\n".join(rpt))
    print("Report written to", RUN_DIR / f'report_{SCRIPT_NAME}.txt')
    log_f.close() 
