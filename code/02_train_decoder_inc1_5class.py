...
"""02_train_decoder_inc1_5class.py
5-class EEGNet decoder for **+1 transitions**.
Classes = landing digits 2-6 (after +1 change).
Data: accuracy-only dataset.
Features: per-channel Z-score (no augmentation).
Outputs: per-fold & overall 5×5 confusion matrices and detailed report.
"""
...
def parse_args():
    pr = argparse.ArgumentParser(formatter_class=argparse.RawDescriptionHelpFormatter,
                                description="inc1 5-class decoder with YAML overrides")
    pr.add_argument('--cfg', type=str)
    for k in DEFAULTS:
        pr.add_argument(f"--{k}")
    pr.add_argument('--set', nargs='*', metavar='KEY=VAL')
    return pr.parse_args()

args = parse_args(); cfg=DEFAULTS.copy(); cfg.update(load_cfg(args.cfg))
explicit={k:getattr(args,k) for k in DEFAULTS if getattr(args,k) is not None}; cfg.update(explicit)
if args.set:
    for kv in args.set:
        k,v=kv.split('=',1)
...
SHIFT_P=cfg['shift_p']; SHIFT_MIN_FRAC=cfg['shift_min_frac']; SHIFT_MAX_FRAC=cfg['shift_max_frac']
SCALE_P=cfg['scale_p']; SCALE_MIN=cfg['scale_min']; SCALE_MAX=cfg['scale_max']
NOISE_P=cfg['noise_p']; NOISE_STD=cfg['noise_std']; CHANNEL_DROP=cfg['channel_drop']; MAX_FOLDS=cfg['max_folds']
...
def load_epochs():
    ...
    pat = re.compile(r"sub-(\d+)_preprocessed")
    eps = []
    for fp in files:
        ep = mne.read_epochs(fp, preload=True, verbose=False)
        sid = int(pat.search(fp.name).group(1))
        ep.metadata['subject'] = sid
        # keep only +1 transitions
        mask = ep.metadata['Condition'].astype(str).isin(INC_SET)
        ep = ep[mask]
        eps.append(ep)
    all_ep = mne.concatenate_epochs(eps)
    cond = all_ep.metadata['Condition'].astype(int)
    landing = (cond % 10).astype(str)  # '2'..'6'
    all_ep.metadata['land_digit'] = landing
    return all_ep
...
def build_tensors(ep):
    ...
    labels = np.array(ep.metadata['land_digit'])
    ...
    return X_t, y, groups, classes

# ---------------- augmentation classes ----------------


class RandomAmpScale:
    def __init__(self, p=SCALE_P, scale_min=SCALE_MIN, scale_max=SCALE_MAX):
        self.p=p; self.scale_min=scale_min; self.scale_max=scale_max
    def __call__(self, eeg: torch.Tensor):
...
class RandomTimeShiftTorch:
    def __init__(self,p,shift_min,shift_max,dim=-1):
    ...
    def __call__(self,eeg:torch.Tensor):
        if self.shift_max>0 and torch.rand(1,device=eeg.device).item()<self.p:
            sh=int(torch.randint(self.shift_min,self.shift_max+1,(1,),device=eeg.device).item())
            if torch.rand(1,device=eeg.device).item()<0.5: sh=-sh
            eeg=torch.roll(eeg,sh,dims=self.dim)
        return eeg


class RandomNoiseTorch:
    def __init__(self,p,std): self.p=p; self.std=std
    def __call__(self,eeg:torch.Tensor):
...
if __name__=='__main__':
    ep_all = load_epochs()
    X_t, y_t, groups, cls = build_tensors(ep_all)
    ...
    for fold,(tr_idx,val_idx) in enumerate(LeaveOneGroupOut().split(np.zeros(len(ds)), np.zeros(len(ds)), groups)):
        print(f"\n=== Fold {fold+1} ===")
        tr_ld=DataLoader(Subset(ds,tr_idx),batch_size=BATCH,shuffle=True)
        val_ld=DataLoader(Subset(ds,val_idx),batch_size=BATCH,shuffle=False)
        cls_w=compute_class_weight('balanced', classes=np.arange(num_classes), y=y_t[tr_idx].numpy())
        ...
        accs.append(best_a)
        y_true=y_t[val_idx].numpy(); y_pred=model(X_t[val_idx].to(DEVICE)).argmax(1).cpu().numpy(); cm=confusion_matrix(y_true,y_pred,labels=np.arange(num_classes)); cm_tot+=cm
        ...
        for i in range(num_classes):
            ...
            j=int(np.argmax(cm_pct[i])); ax.add_patch(Rectangle((j,i),1,1,fill=False,edgecolor='red',lw=2.5))
        plt.tight_layout(); plt.savefig(RUN_DIR/f'fold{fold+1}_confusion_{SCRIPT_NAME}.png'); plt.close()
    # overall
    cm_pct_tot=cm_tot/cm_tot.sum(axis=1,keepdims=True)*100; annot=np.round(cm_pct_tot,1)
    plt.figure(figsize=(6,5)); ax=sns.heatmap(cm_pct_tot,annot=annot,fmt='.1f',cmap='Blues',xticklabels=[f'land {d}' for d in cls],yticklabels=[f'land {d}' for d in cls]); plt.xticks(rotation=0); plt.yticks(rotation=0)
    ...
    summary={
        ...
        "device":str(DEVICE),
        "task":"inc1_5class",
        "classes": [str(i) for i in range(2,7)], # Classes 2-6
        "hyper": cfg.copy(),
        "fold_metrics":[]
    }
    summary.update({
