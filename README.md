...
Live epoch logs appear when you call `python` directly.  If you prefer to use
`conda run` add `-u` (`python -u …`) to force unbuffered output.
...
```yaml
dataset_dir: data_dual_stream/acc_1_dataset
